...
int raw_send (int sk, char *buffer, char *ip_address)
{
   ...
   if (sendto (sk, buffer, (size_t)msg_len, 0, (struct sockaddr *) &ska, sizeof(ska)) != msg_len)
   {
...
int raw_receive_and_get_sender_info (int sk, char *buffer, char *ip_address)
{
   ...
   if ((dim = (int)recvfrom (sk, buffer, BUFSIZ, 0,
...
int raw_reply (int sk, char *buffer)
{
   ...
   if (sendto (sk, buffer, (size_t)msg_len, 0, 
       (struct sockaddr *) &reply_to_socket_address, 
       (socklen_t)len_of_reply_to_socket_address) != msg_len)
   {
